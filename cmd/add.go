...
func init() {
	...
	addCmd.Flags().StringVar(&addID, "app-id", "", "app ID (reverse-DNS); derived from the manifest when omitted")
	addCmd.Flags().StringVar(&addID, "id", "", "deprecated alias for --app-id")
	_ = addCmd.Flags().MarkDeprecated("id", "please use --app-id instead")
